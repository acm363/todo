...
describe ('TodolistController', () => {
    ...
    it ('should throw error when non existing todoId is given for findOne method.', async () => {
      // Given.
      const nonExistingTodoId = '';
      todoListServiceMock.findOne.mockImplementationOnce((todoId) => {
        throw new NotFoundException(`Todo with todoId #${todoId} not found in the database!`);
      });

      /*
         When.
         If we get the Todo by await, the not find exception will be throw in this context, so we just save the promise and make
         an await on the expected statement.
        */
